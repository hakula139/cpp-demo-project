...
enum class ErrorSeverity { kInfo, kWarning, kError, kCritical };
...
class BaseException : public std::exception {
 public:
  ...
  [[nodiscard]] auto GetFormattedMessage() const -> std::string {
    return std::format("{}:{} in {}: {}", location_.file_name(), location_.line(),
...
class ValidationException : public BaseException {
 public:
  explicit ValidationException(std::string_view message, std::string_view field_name = "",
                               std::source_location location = std::source_location::current())
      : BaseException(message, ErrorSeverity::kError, location), field_name_(field_name) {}

  [[nodiscard]] auto GetFieldName() const noexcept -> std::string_view { return field_name_; }
...
class ResourceException : public BaseException {
 public:
  explicit ResourceException(std::string_view message, std::string_view resource_name = "",
                             std::source_location location = std::source_location::current())
      : BaseException(message, ErrorSeverity::kError, location), resource_name_(resource_name) {}

  [[nodiscard]] auto GetResourceName() const noexcept -> std::string_view { return resource_name_; }
