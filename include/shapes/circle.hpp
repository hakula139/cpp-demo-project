...
class Circle final : public Shape {
 public:
  ...
  [[nodiscard]] auto GetPerimeter() const noexcept -> std::expected<double, ShapeError> override {
    ...
    return K_PERIMETER_MULTIPLIER * std::numbers::pi * radius_;  // NOLINT(bugprone-narrowing-conversions)
  }
  ...
  [[nodiscard]] constexpr auto GetRadius() const noexcept -> double {
    return radius_;
  }

  auto operator<=>(const Circle& other) const noexcept {
