...
TEST_CASE("Container basic operations", "[containers][basic]") {
  ...
  SECTION("Element access") {
    ...
    REQUIRE(result1->get() == 10);

    auto result2 = container.At(1);
    REQUIRE(result2.has_value());
    REQUIRE(result2->get() == 20);
...
TEST_CASE("Container advanced operations", "[containers][advanced]") {
  ...
  SECTION("Emplace elements") {
    ...
    container.Emplace(5, '!');  // std::string constructor with count and character
...
TEMPLATE_TEST_CASE("Container with different types", "[containers][template]", int, double,
                   std::string) {
